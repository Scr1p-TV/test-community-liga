...
window.handleLogin = async function() {
  ...
  if (!team) {
    err.textContent = "Team nicht gefunden. Frage den Commissioner.";
    return;
  }
...
window.saveDraftConfig = async function() {
  ...
  teamRows.forEach(row => {
    const n = row.querySelector(".team-name-input").value.trim();
    const p = row.querySelector(".team-pw-input").value.trim();
...
function buildSnakeOrder(teamNames, rounds, snake) {
  ...
  for (let r = 0; r < rounds; r++) {
    const round = (snake && r % 2 === 1) ? [...teamNames].reverse() : [...teamNames];
    order.push(...round);
  }
  return order;
}
...
function enterDraft() {
  ...
  info.innerHTML = currentUser.isCommissioner
...
function subscribeToFirebase() {
  ...
  onValue(ref(db, "draftState"), snap => {
    ...
    const round    = Math.ceil(state.currentPick / numTeams);
  ...
  onValue(ref(db, "draftPicks"), snap => {
    const data  = snap.val() || {};
    const picks = Object.values(data).map((p, i) => Object.assign({}, p, { pickNumber: i + 1 }));

    if (picks.length > lastPickCount && lastPickCount > 0) {
      showPickAnimation(picks[picks.length - 1]);
    }
...
function startLocalTimer() {
  timerInterval = setInterval(() => {
    ...
    syncTimerDisplay();
    if (timerValue % 5 === 0) {
      update(ref(db, "draftState"), { timerValue: timerValue });
    }
  }, 1000);
}
...
async function handleTimerExpired() {
  ...
  if (pickQueue.length > 0) {
    const dSnap   = await get(ref(db, "draftPicks"));
    const drafted = Object.values(dSnap.val() || {}).map(p => p.player);
    const next    = pickQueue.find(p => !drafted.includes(p));
    if (next) {
...
async function advancePick() {
  ...
  const newTimer = draftConfig ? draftConfig.timerSeconds : 90;
  timerValue = newTimer;
...
function renderDraftBoard(picks) {
  ...
  const numTeams  = draftConfig ? draftConfig.teams.length : 1;
  const numRounds = draftConfig ? draftConfig.numRounds : 1;
  ...
  if (tabsEl.children.length !== numRounds) {
    ...
    for (let r = 1; r <= numRounds; r++) {
      ...
      tab.textContent = "R" + r;
      tab.onclick = (function(round) {
        return function() {
          activeRound = round;
          document.querySelectorAll(".round-tab").forEach(function(t, i) {
            t.classList.toggle("active", i + 1 === round);
          });
  ...
  picks.filter(p => p.round === activeRound).forEach(pick => {
    const row = document.createElement("tr");
    if (pick.team === (currentUser && currentUser.team)) row.className = "my-pick";
...
function renderPlayers(draftedPlayers) {
  ...
  const search = (document.getElementById("playerSearch") ? document.getElementById("playerSearch").value : "").toLowerCase();
  ...
  available.forEach(player => {
    ...
    div.innerHTML =
      ...
      "<div class='player-actions'>" +
        "<button class='btn-queue" + (inQueue ? " queued" : "") + "' onclick=\"toggleQueue('" + player.replace(/'/g, "\\'") + "')\">&#9733; Queue</button>" +
        "<button class='btn-draft' onclick=\"handleDraftClick('" + player.replace(/'/g, "\\'") + "')\">Draften</button>" +
      "</div>";
    el.appendChild(div);
  });
}
...
function renderPickQueue() {
  ...
  pickQueue.forEach((player, i) => {
    ...
    item.innerHTML =
      ...
      "<button class='queue-remove' onclick=\"removeFromQueue('" + player.replace(/'/g, "\\'") + "')\">×</button>";
    el.appendChild(item);
  });
}
...
function renderDraftOrder(state) {
  ...
  state.order.slice(currentIdx, currentIdx + 6).forEach(function(team, i) {
    const item = document.createElement("div");
    item.className = "order-item" + (i === 0 ? " on-clock" : "");
    item.innerHTML =
      "<div class='order-dot'></div>" +
      "<div class='order-name'>" + team + "</div>" +
      "<div class='order-pick-num'>Pick " + (state.currentPick + i) + "</div>";
    el.appendChild(item);
  });
}
